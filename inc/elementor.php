...
/**
 * Preview Styles & Scripts Isolation
 * Enqueue theme CSS and JS into preview iframe
 */
function bsd_elementor_preview_styles() {
    ...
    wp_enqueue_style('bsd-tokens', get_template_directory_uri() . '/assets/css/tokens.css', array(), BSD_VERSION);
    wp_enqueue_style('bsd-widgets', get_template_directory_uri() . '/assets/css/widgets.css', array('bsd-tokens'), BSD_VERSION);
}
add_action('elementor/preview/enqueue_styles', 'bsd_elementor_preview_styles');

function bsd_elementor_preview_scripts() {
    wp_enqueue_script('bsd-main', get_template_directory_uri() . '/assets/js/main.js', array(), BSD_VERSION, true);
    wp_localize_script('bsd-main', 'bsdData', array(
        'ajaxUrl' => admin_url('admin-ajax.php'),
        'nonce'   => wp_create_nonce('bsd_contact_nonce'),
    ));
}
add_action('elementor/preview/enqueue_scripts', 'bsd_elementor_preview_scripts');
